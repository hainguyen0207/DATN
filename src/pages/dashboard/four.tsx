...
export default function Page() {
  ...
  const handleAddNewScan = () => {
    ...
    localStorage.setItem('sql-scan-history', newDataStr);
    setHistory(newData);
    navigate(`?id=${uuid}`);
  };
  ...
  const handleScan = async () => {
    if (param && requestFile) {
      try {
        ...
        const res = await axiosInstance.post('/sqli-scanner', formData, {
          headers: {
        ...
        if (res.status === 200) {
          ...
          if (id) {
            newData = history.map((item) => {
              if (item.id === id) {
                ...
                item.scanned = true;
                item.downloadLink = res?.data?.result?.log_path || '';
              }
              return item;
            });
          } else {
            ...
            newData = [
              {
                ...
                scanned: true,
                downloadLink: res?.data?.result?.log_path || '',
              },
              ...history,
            ];
          }
          ...
          setDownloadLink(res?.data?.result?.log_path || '');
        } else {
  ...
  useEffect(() => {
    const dataStr = localStorage.getItem('sql-scan-history') || '[]';
...
const PathView = ({ data, downloadLink }: PathViewProps) => {
  const handleDownload = () => {
    if (downloadLink) {
      ...
      link.href = 'http://127.0.0.1:2727/log-sqli-scanner';
      link.download = 'sqli_log.json';
      link.target = '_blank';
